...
#'############# SIMULATION 4 - MIXTURE OF 4 GAUSSIAN DISTRIBUTIONS ###########'#
#'############################################################################'#

## LIBRARIES AND FUNCTIONS -----------------------------------------------------
source("libraries_4.R")
...
for (N in NN) {
  ...
  for(time_index in 1:length(t)){
    ...
    # First Plot: Sample
    name <- paste0("pictures/", N, "_data/sample/sim4_sample_intensity_", N, "data_", t[time_index])
    plot.sample(locations[abs(times-t[time_index])<h,], name)
      
    # Second Plot: True Density at t[time_index]
    name <- paste0("pictures/", N, "_data/true_instant/sim4_true_instant_intensity_", N, "data_", t[time_index])
    ...
    if(sum(abs(t[time_index] - t_discrete) < 1e-06)){
      ...
      name <- paste0("pictures/", N, "_data/true_discrete/sim4_true_discrete_intensity_", N, "data_", t[time_index])
      plot.density(mesh.eval$nodes[,1], mesh.eval$nodes[,2], true_discrete[,time_index_discrete],
                   max_range = M,
                   filename = name, colorscale = "Jet")
    ...
    name <- paste0("pictures/", N, "_data/STDEPDE/sim4_STDEPDE_intensity_", N, "data_", t[time_index])
    ...
    if(sum(abs(t[time_index] - t_discrete) < 1e-06)){
      ...
      name <- paste0("pictures/", N, "_data/LGCP/sim4_LGCP_intensity_", N, "data_", t[time_index])
    ...
    name <- paste0("pictures/", N, "_data/INLA/sim4_INLA_intensity_", N, "data_", t[time_index])
    ...
    name <- paste0("pictures/", N, "_data/INLA_aggregated/sim4_INLA_aggregated_intensity_", N, "data_", t[time_index])
    ...
    for(time_index in 1:length(t)) {
      ...
      string <- paste0(paste0("pictures/", N, "_data/time_bar/sim4_time_bar_intensity_",time_index,".pdf"))
    ...
    ### LEGEND -----------------------------------------------------------------
    pdf(paste0("pictures/", N, "_data/sim4_legend_intensity.pdf"), family = "serif", width = 11, height = 3)
  ...
  ### ERRORS [FIGURE F.14] -----------------------------------------------------
  {
    ...
    pdf(paste0("pictures/", N, "_data/sim4_errors_intensity.pdf"), family = "serif", width = 12, height = 5.3)
  ...
  pdf(paste0("pictures/", N, "_data/sim4_CPUtimes_intensity.pdf"), family = "serif", width = 12, height = 5.3)
  #x11(width = 12, height = 5.3)
  par(mfrow = c(1,3), mai = c(1.75,0.35,0.5,0.15))
  plot.new()
  ...
  ### MESH VISUALIZATION -------------------------------------------------------
  name <- paste0("pictures/sim4_mesh_intensity")
  plot_mesh(mesh, name)
  
  name <- paste0("pictures/sim4_mesh_eval_intensity")
