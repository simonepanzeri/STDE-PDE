...
#'########### SIMULATION 6 - MIXTURE OF DENSITIES OVER 2.5D DOMAIN ###########'#
#'############################################################################'#

## LIBRARIES AND FUNCTIONS -----------------------------------------------------
source("libraries_6.R")
...
vertices <- read.table("mesh/simulation6.surface.vertices.txt")
triangles <- read.table("mesh/simulation6.surface.triangles.txt")
...
for(N in NN){
  ...
  ## VISUALIZATION -------------------------------------------------------------
  {
    ### ESTIMATES [FIGURE H.17 and H.18] ---------------------------------------
    ...
    for(time_index in 1:length(t)){
      ...
      # First Plot: Sample
      string <- paste0("pictures/", N, "_data/sample/sim6_sample_", t[time_index], ".png")
      ...
      string <- paste0("pictures/", N, "_data/true_instant/sim6_true_instant_", t[time_index], ".png")
      plot.FEM(FEM(true_instant[,time_index], FEMbasis.eval), M)
      snapshot3d(string, fmt = "png", width = 600, height = 400, webshot = rgl.useNULL())
      rgl.close()
        
      # Third Plot: STDE-PDE Estimated Density at t[time_index]
      string <- paste0("pictures/", N, "_data/STDEPDE/sim6_STDEPDE_", t[time_index], ".png")
      plot.FEM(FEM(mean_sol_STDEPDE[,time_index], FEMbasis.eval), M)
      snapshot3d(string, fmt = "png", width = 600, height = 400, webshot = rgl.useNULL())
      rgl.close()
        
      # Fourth Plot: KNNSTDE Estimated Density at t[time_index]
      string <- paste0("pictures/", N, "_data/KNNSTDE-separable/sim6_KNNSTDE_separable_", t[time_index], ".png")
    ...
    ## LEGEND [FIGURE H.18] ----------------------------------------------------
    pdf(paste0("pictures/", N, "_data/sim6_legend.pdf"), family = "serif", width = 11, height = 3)
    ...
    ## ERRORS [FIGURE H.19] ----------------------------------------------------
    {
      ...
      pdf(paste0("pictures/", N, "_data/sim6_errors.pdf"), family = "serif", width = 12, height = 5.3)
    ...
    ## CPU TIMES ---------------------------------------------------------------
    pdf(paste0("pictures/", N, "_data/sim6_CPUtimes.pdf"), family = "serif", width = 12, height = 5.3)
    #x11(width = 12, height = 5.3)
    par(mfrow = c(1,3), mai = c(1.75,0.35,0.5,0.15))
    plot.new()
    ...
    ## MESH --------------------------------------------------------------------
    string <- paste0("pictures/", N, "_data/sim6_mesh.png")
    plot.mesh(mesh)
    snapshot3d(string, fmt = "png", width = 600, height = 400, webshot = rgl.useNULL())
    rgl.close()
    
    string <- paste0("pictures/", N, "_data/sim6_mesh.eval.png")
    plot.mesh(mesh.eval)
    snapshot3d(string, fmt = "png", width = 600, height = 400, webshot = rgl.useNULL())
    rgl.close()
      
    ## DOMAIN ------------------------------------------------------------------
    string <- paste0("pictures/", N, "_data/sim6_domain.png")
    ...
    for(time_index in 1:length(t)){
      ...
      string <- paste0("pictures/", N, "_data/time_bar/sim6_time_bar", time_index, ".pdf")
