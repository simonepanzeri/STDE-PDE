...
for (N in NN) {
  ...
  for(proc in 1:processes){
    ...
    m.prj <- lapply(1:k, function(j) {
      ...
      r <- INLA::inla.mesh.project(prj, exp(res_INLA$summary.fix$mean + res_INLA$summary.ran$s$mean[1:m + (j - 1) * m]) * st.vol[1:m + (j - 1) * m])
      r[is.na(ov)] <- NA
      return(r) 
    })
    ...
    t.mean <- lapply(1:k, function(j) {
      ...
      z.j <- exp(res_INLA_aggregated$summary.fix$mean + res_INLA_aggregated$summary.ran$s$mean[idx_INLA$s.group == j]) * w.areas
    ...
    for (time_index in 1:length(t)) {
      ...
      if(proc == 1){
        ...
        true_instant[,time_index] <- dens.func(mesh.eval$nodes, t[time_index])/domain_area
        ...
        if(sum(abs(t[time_index] - t_discrete) < 1e-06)){
          ...
          for(time_subinterval in subinterval){
            coeff <- dens.func(mesh.eval$nodes, time_subinterval)/domain_area
      ...
      evaluation_STDEPDE <- exp(evaluation_STDEPDE)*N
      ...
      if(sum(abs(t[time_index] - t_discrete) < 1e-06)){
        ...
        grid_LGCP_area <- abs(grid_LGCP[1,1]-grid_LGCP[2,1]) * abs(grid_LGCP[1,2]-grid_LGCP[1+n_LGCP,2])
        evaluation_LGCP <- evaluation_LGCP*grid_LGCP_area
      ...
      Hdist_STDEPDE[proc] <- Hdist_STDEPDE[proc] + hellinger.distance(evaluation_STDEPDE, true_instant[,time_index])/length(t)
      ...
      Hdist_INLA[proc] <- Hdist_INLA[proc] + hellinger.distance(evaluation_INLA, true_instant[,time_index])/length(t)
      ...
      Hdist_INLA_aggregated[proc] <- Hdist_INLA_aggregated[proc] + hellinger.distance(evaluation_INLA_aggregated, true_instant[,time_index])/length(t)
      ...
      if(sum(abs(t[time_index] - t_discrete) < 1e-06)){
        ...
        Hdist_LGCP[proc] <- Hdist_LGCP[proc] + hellinger.distance(evaluation_LGCP, true_discrete[,time_index_discrete])/length(t_discrete)
  ...
  estimates <- list(mean_sol_STDEPDE, mean_sol_LGCP, mean_sol_INLA, mean_sol_INLA_aggregated)
  save(estimates, file = paste0("output/estimates_intensity_", N, "data.rda"))
  
  mise <- list(mise_STDEPDE, mise_LGCP, mise_INLA, mise_INLA_aggregated)
  save(mise, file = paste0("output/mise_intensity_", N, "data.rda"))
  
  KLdist <- list(KLdist_STDEPDE, KLdist_LGCP, KLdist_INLA, KLdist_INLA_aggregated)
  save(KLdist, file = paste0("output/KLdist_intensity_", N, "data.rda"))
  
  Hdist <- list(Hdist_STDEPDE, Hdist_LGCP, Hdist_INLA, Hdist_INLA_aggregated)
  save(Hdist, file = paste0("output/Hdist_intensity_", N, "data.rda"))
  
  CPUtimes <- list(CPUtimes_STDEPDE, CPUtimes_LGCP, CPUtimes_INLA, CPUtimes_INLA_aggregated)
  save(CPUtimes, file = paste0("output/CPUtimes_intensity_", N, "data.rda"))
  
  base::save.image(paste0("output/workspace_intensity_", N, "data.RData"))
  ...
  M <- max(max(true_instant, na.rm = TRUE),
           ...
           max(mean_sol_INLA_aggregated, na.rm = TRUE), na.rm = TRUE)
            
  dir.create(file.path(getwd(), "pictures_intensity"), showWarnings = FALSE)
  dir.create(file.path(getwd(), paste0("pictures_intensity/", N, "_data")), showWarnings = FALSE)
  dir.create(file.path(getwd(), paste0("pictures_intensity/", N, "_data/sample")), showWarnings = FALSE)
  dir.create(file.path(getwd(), paste0("pictures_intensity/", N, "_data/true_instant")), showWarnings = FALSE)
  dir.create(file.path(getwd(), paste0("pictures_intensity/", N, "_data/true_discrete")), showWarnings = FALSE)
  dir.create(file.path(getwd(), paste0("pictures_intensity/", N, "_data/STDEPDE")), showWarnings = FALSE)
  dir.create(file.path(getwd(), paste0("pictures_intensity/", N, "_data/LGCP")), showWarnings = FALSE)
  dir.create(file.path(getwd(), paste0("pictures_intensity/", N, "_data/INLA")), showWarnings = FALSE)
  dir.create(file.path(getwd(), paste0("pictures_intensity/", N, "_data/INLA_aggregated")), showWarnings = FALSE)
  dir.create(file.path(getwd(), paste0("pictures_intensity/", N, "_data/time_bar")), showWarnings = FALSE)
  
  for(time_index in 1:length(t)){
    t_img <- proc.time()
      
    # First Plot: Sample
    # name <- paste0("pictures_intensity/", N, "_data/sample/sim1_sample_intensity_", N, "data_", t[time_index])
    # plot_sample(locations[abs(times-t[time_index])<h,], name)
      
    # Second Plot: True Density at t[time_index]
    name <- paste0("pictures_intensity/", N, "_data/true_instant/sim1_true_instant_intensity_", N, "data_", t[time_index])
    plot_density(mesh.eval$nodes[,1], mesh.eval$nodes[,2], true_instant[,time_index],
                 max_range = M,
                 filename = name, colorscale = "Jet")
    #plot_interactive_density(FEM(true_instant[,time_index], FEMbasis.eval))
      
    # name <- paste0("pictures_intensity/", N, "_data/true_discrete/sim1_true_discrete_intensity_", N, "data_", t[time_index])
    # plot_density(mesh.eval$nodes[,1], mesh.eval$nodes[,2], true_discrete[,time_index],
    #              max_range = M,
    #              filename = name, colorscale = "Jet")
    #plot_interactive_density(FEM(true_discrete[,time_index], FEMbasis.eval))
      
    # Third Plot: STDE-PDE Estimated Density at t[time_index]
    name <- paste0("pictures_intensity/", N, "_data/STDEPDE/sim1_STDEPDE_intensity_", N, "data_", t[time_index])
    plot_density(mesh.eval$nodes[,1], mesh.eval$nodes[,2], mean_sol_STDEPDE[,time_index],
                 max_range = M,
                 filename = name, colorscale = "Jet")
    #plot_interactive_density(FEM(mean_sol_STDEPDE[,time_index], FEMbasis.eval))
    
    if(sum(abs(t[time_index] - t_discrete) < 1e-06)){
      time_index_discrete <- which(abs(t[time_index] - t_discrete) < 1e-06)
        
      # Fourth Plot: LGCP Estimated Density at t[time_index]
      name <- paste0("pictures_intensity/", N, "_data/LGCP/sim1_LGCP_intensity_", N, "data_", t[time_index])
      plot_density(mesh.eval$nodes[,1], mesh.eval$nodes[,2], mean_sol_LGCP[,time_index_discrete],
                   max_range = M,
                   filename = name, colorscale = "Jet")
      #plot_interactive_density(FEM(mean_sol_LGCP[,time_index_discrete], FEMbasis.eval))
        
    }
      
    # Fifth Plot: INLA Estimated Density at t[time_index]
    name <- paste0("pictures_intensity/", N, "_data/INLA/sim1_INLA_intensity_", N, "data_", t[time_index])
    plot_density(mesh.eval$nodes[,1], mesh.eval$nodes[,2], mean_sol_INLA[,time_index],
                 max_range = M,
                 filename = name, colorscale = "Jet")
    #plot_interactive_density(FEM(mean_sol_INLA[,time_index], FEMbasis.eval))
      
    # Sixth Plot: INLA-aggregated Estimated Density at t[time_index]
    name <- paste0("pictures_intensity/", N, "_data/INLA_aggregated/sim1_INLA_aggregated_intensity_", N, "data_", t[time_index])
    plot_density(mesh.eval$nodes[,1], mesh.eval$nodes[,2], mean_sol_INLA_aggregated[,time_index],
                 max_range = M,
                 filename = name, colorscale = "Jet")
    #plot_interactive_density(FEM(mean_sol_INLA_aggregated[,time_index], FEMbasis.eval))
    ...
    for(time_index in 1:length(t)) {
      ...
      string <- paste0(paste0("pictures_intensity/", N, "_data/time_bar/sim1_time_bar_intensity_",time_index,".pdf"))
    ...
    ### LEGEND -----------------------------------------------------------------
    pdf(paste0("pictures_intensity/", N, "_data/sim1_legend_intensity.pdf"), family = "serif", width = 11, height = 3)
  ...
  ### ERRORS [FIGURE G.15] -----------------------------------------------------
  {
    ...
    pdf(paste0("pictures_intensity/", N, "_data/sim1_errors_intensity.pdf"), family = "serif", width = 12, height = 5.3)
    ...
    par(new = TRUE)
    boxplot(mise_STDEPDE, mise_LGCP, mise_INLA, mise_INLA_aggregated,
            names = c("STDE-PDE", "LGCP", "INLA", "INLA-agg"), main = TeX("$err_{L^2}$", bold=T),
            col = brewer.pal(8, "YlGnBu")[1:4], cex.lab = 2, cex.axis = 2, cex.main = 2, las = 3)
    dev.off()
  } 

  ### CPU TIMES VISUALIZATION --------------------------------------------------
  pdf(paste0("pictures_intensity/", N, "_data/sim1_CPUtimes_intensity.pdf"), family = "serif", width = 12, height = 5.3)
  #x11(width = 12, height = 5.3)
  par(mfrow = c(1,3), mai = c(1.75,0.35,0.5,0.15))
  plot.new()
  ...
  par(new = TRUE)
  boxplot(CPUtimes_STDEPDE, CPUtimes_LGCP, CPUtimes_INLA, CPUtimes_INLA_aggregated,
          names = c("STDE-PDE", "LGCP", "INLA", "INLA-agg"), main = "CPU TIMES [seconds]",
          col = brewer.pal(8, "YlGnBu"), cex.lab = 2, cex.axis = 2, cex.main = 2, las = 3)
  dev.off()

  ### MESH VISUALIZATION -------------------------------------------------------
  name <- paste0("pictures_intensity/sim1_mesh_intensity")
  plot_mesh(mesh, name)
  
  name <- paste0("pictures_intensity/sim1_mesh_eval_intensity")
  plot_mesh(mesh.eval, name)
