#'############################################################################'#
#'######## SIMULATION 3 - INHOMOGENEOUS POISSON PROCESS ON EASTBOURNE ########'#
#'############################################################################'#
...
dir.create(paste0(getwd(),"/simulation_3"))
setwd(paste0(getwd(),"/simulation_3"))
foldername <- getwd()
...
for(N in NN){
  ...
  w <- compute.weight(fun = inhomogeneous_intensity5, f = FEMbasis.eval, N = N)
  
  # Generate the Data (Homogeneous Poisson Process)
  cat("Generating the data...\n")
  if(!file.exists(paste0(getwd(),"/data/dataset_",processes,"x",N,".RData"))){
    
    dataset <- rpoistlpp(lambda = inhomogeneous_intensity5, lmax = NULL,
  ...
  for(proc in 1:processes){
    ...
    for(time_index in 1:length(t)) {
      ...
      if(proc == 1){
        ...
        true_intensity[,time_index] <- inhomogeneous_intensity5(mesh.eval$nodes[,1], mesh.eval$nodes[,2], t[time_index])
      ...
      evaluation_STLNPP <- as.linfun(solution_STLNPP[[idx]])(mesh.eval$nodes[,1], mesh.eval$nodes[,2]) #attr(solution_STLNPP, "netint") * attr(solution_STLNPP, "tempden")$y[idx]
  ...
  ## VISUALIZATION -------------------------------------------------------------
  {
    ...
    for(time_index in 1:length(t)){
      ...
      plotname <- paste0("pictures/", N, "_data/SAMPLE/sim3_SAMPLE_", N, "data_", time_index, ".pdf")
      ggsave(filename = plotname, plot = last_plot(), width = 8, height = 8)
      
      for(col in color_palette) {
        
        # Second Plot: True Density at t[time_index]
        plotname <- paste0("pictures/", N, "_data/TRUE_INTENSITY/sim3_TRUE_INTENSITY_", N, "data_", col, "_", time_index)
    ...
    for(col in color_palette) {
      ...
      pdf(paste0("pictures/", N, "_data/sim3_legend_", col, ".pdf"), family = "serif", width = 11, height = 3)
      ...
      axis(1, at = c(0,25,50,75,100), labels = c("0", "0.4", "0.8", "1.2", "1.6"),
    ...
    ## ERRORS ------------------------------------------------------------------
    {
      ...
      pdf(paste0("pictures/", N, "_data/sim3_errors.pdf"), family = "serif", width = 11.75, height = 5.25)
    ...
    
    {
      ...
      pdf(paste0("pictures/", N, "_data/sim3_errors_zoom.pdf"), family = "serif", width = 11.75, height = 5.25)
      #x11(width = 11.75, height = 5.25)
      par(mfrow = c(1,3), mai = c(1.75,0.75,0.5,0.15))
      plot.new()
      boxplot(mise_STDEPDE, mise_EQUAL_SPLIT, mise_HEAT, mise_STLNPP,
    ...
    ## CPU TIMES ---------------------------------------------------------------
    pdf(paste0("pictures/", N, "_data/sim3_CPUtimes.pdf"), family = "serif", width = 11.75, height = 5.25)
    #x11(width = 11.75, height = 5.25)
    par(mfrow = c(1,3), mai = c(1.75,0.75,0.5,0.15))
    plot.new()
    ...
    plot(mesh, linewidth = 1) + geom_point(data = data.frame(x = mesh$nodes[,1], y = mesh$nodes[,2]),
                                           aes(x = x, y = y), color = "red3", size = 2)
    ggsave(filename = paste0("pictures/", N, "_data/sim3_mesh.pdf"),
    ...
    plot(mesh.eval, linewidth = 1) + geom_point(data = data.frame(x = mesh.eval$nodes[,1], y = mesh.eval$nodes[,2]),
                                                aes(x = x, y = y), color = "red3", size = 2)
    ggsave(filename = paste0("pictures/", N, "_data/sim3_mesh_eval.pdf"),
           plot = last_plot(), width = 8, height = 8)
    
    ## TIME BAR ----------------------------------------------------------------
    for(time_index in 1:length(t)) {
      string <- paste0(paste0("pictures/", N, "_data/TIME_BAR/sim3_time_bar_",time_index,".pdf"))
...
# lambda_space <- c(100, 10, 1, 1e-1, 1e-2, 1e-3, 1e-4)
# lambda_time <- c(100, 10, 1, 1e-1, 1e-2, 1e-3, 1e-4)

lambda_space <- c(0.025, 0.01, 0.005, 0.001, 0.0005, 0.0001)
lambda_time <- c(50, 25, 10, 1, 0.1, 0.01)
