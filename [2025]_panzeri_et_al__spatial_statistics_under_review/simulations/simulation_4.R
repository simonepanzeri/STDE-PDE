#'############################################################################'#
#'######### SIMULATION 4 - HOMOGENEOUS POISSON PROCESS ON EASTBOURNE #########'#
#'############################################################################'#
...
dir.create(paste0(getwd(),"/simulation_4"))
setwd(paste0(getwd(),"/simulation_4"))
foldername <- getwd()
...
for(N in NN){
  ...
  if(!file.exists(paste0(getwd(),"/data/dataset_",processes,"x",N,".RData"))){
    ...
    dataset <- rpoistlpp(lambda = homogeneous_intensity3, lmax = NULL,
  ...
  for(proc in 1:processes){
    ...
    for(time_index in 1:length(t)) {
      ...
      if(proc == 1){
        ...
        true_intensity[,time_index] <- homogeneous_intensity3(mesh.eval$nodes[,1], mesh.eval$nodes[,2], t[time_index])
  ...
  ## VISUALIZATION -------------------------------------------------------------
  {
    ...
    for(time_index in 1:length(t)){
      ...
      plotname <- paste0("pictures/", N, "_data/SAMPLE/sim4_SAMPLE_", N, "data_", time_index, ".pdf")
      ggsave(filename = plotname, plot = last_plot(), width = 8, height = 8)
      
      for(col in color_palette) {
        
        # Second Plot: True Density at t[time_index]
        plotname <- paste0("pictures/", N, "_data/TRUE_INTENSITY/sim4_TRUE_INTENSITY_", N, "data_", col, "_", time_index)
    ...
    for(col in color_palette) {
      ...
      pdf(paste0("pictures/", N, "_data/sim4_legend_", col, ".pdf"), family = "serif", width = 11, height = 3)
    ...
    ## ERRORS ------------------------------------------------------------------
    {
      ...
      pdf(paste0("pictures/", N, "_data/sim4_errors.pdf"), family = "serif", width = 11.75, height = 5.25)
    ...
    
    {
      ...
      pdf(paste0("pictures/", N, "_data/sim4_errors_zoom.pdf"), family = "serif", width = 11.75, height = 5.25)
      #x11(width = 11.75, height = 5.25)
      par(mfrow = c(1,3), mai = c(1.75,0.75,0.5,0.15))
      plot.new()
      boxplot(mise_STDEPDE, mise_EQUAL_SPLIT, mise_HEAT, mise_STLNPP,
    ...
    ## CPU TIMES ---------------------------------------------------------------
    pdf(paste0("pictures/", N, "_data/sim4_CPUtimes.pdf"), family = "serif", width = 11.75, height = 5.25)
    #x11(width = 11.75, height = 5.25)
    par(mfrow = c(1,3), mai = c(1.75,0.75,0.5,0.15))
    plot.new()
    ...
    plot(mesh, linewidth = 1) + geom_point(data = data.frame(x = mesh$nodes[,1], y = mesh$nodes[,2]),
                                           aes(x = x, y = y), color = "red3", size = 2)
    ggsave(filename = paste0("pictures/", N, "_data/sim4_mesh.pdf"),
    ...
    plot(mesh.eval, linewidth = 1) + geom_point(data = data.frame(x = mesh.eval$nodes[,1], y = mesh.eval$nodes[,2]),
                                                aes(x = x, y = y), color = "red3", size = 2)
    ggsave(filename = paste0("pictures/", N, "_data/sim4_mesh_eval.pdf"),
           plot = last_plot(), width = 8, height = 8)
    
    ## TIME BAR ----------------------------------------------------------------
    for(time_index in 1:length(t)) {
      string <- paste0(paste0("pictures/", N, "_data/TIME_BAR/sim4_time_bar_",time_index,".pdf"))
...
lambda_space <- c(0.3, 0.2, 0.15, 0.125, 0.075, 0.05, 0.025)
lambda_time <- c(10, 15, 20, 25, 50, 75, 100)
