#'############################################################################'#
#'######## SIMULATION 1 - INHOMOGENEOUS POISSON PROCESS ON EASTBOURNE ########'#
#'############################################################################'#
...
dir.create(paste0(getwd(),"/simulation_1"))
setwd(paste0(getwd(),"/simulation_1"))
foldername <- getwd()
...
for(N in NN){
  ...
  w <- compute.weight(fun = inhomogeneous_intensity1, f = FEMbasis.eval, N = N)
  
  # Generate the Data (Inhomogeneous Poisson Process)
  cat("Generating the data...\n")
  if(!file.exists(paste0(getwd(),"/data/dataset_",processes,"x",N,".RData"))){
    
    dataset <- rpoistlpp(lambda = inhomogeneous_intensity1, lmax = NULL,
  ...
  for(proc in 1:processes){
    ...
    for(time_index in 1:length(t)) {
      ...
      if(proc == 1){
        ...
        true_intensity[,time_index] <- inhomogeneous_intensity1(mesh.eval$nodes[,1], mesh.eval$nodes[,2], t[time_index])
  ...
  ## VISUALIZATION -------------------------------------------------------------
  {
    ...
    for(time_index in 1:length(t)){
      ...
      plotname <- paste0("pictures/", N, "_data/SAMPLE/sim1_SAMPLE_", N, "data_", time_index, ".pdf")
      ggsave(filename = plotname, plot = last_plot(), width = 8, height = 8)

      for(col in color_palette) {
        
        # Second Plot: True Density at t[time_index]
        plotname <- paste0("pictures/", N, "_data/TRUE_INTENSITY/sim1_TRUE_INTENSITY_", N, "data_", col, "_", time_index)
    ...
    for(col in color_palette) {
      ...
      pdf(paste0("pictures/", N, "_data/sim1_legend_", col, ".pdf"), family = "serif", width = 11, height = 3)
      ...
      axis(1, at = c(0,25,50,75,100), labels = c("0", "0.3", "0.6", "0.9", "1.2"),
    ...
    ## ERRORS ------------------------------------------------------------------
    {
      ...
      pdf(paste0("pictures/", N, "_data/sim1_errors.pdf"), family = "serif", width = 11.75, height = 5.25)
    ...
    
    {
      ...
      pdf(paste0("pictures/", N, "_data/sim1_errors_zoom.pdf"), family = "serif", width = 11.75, height = 5.25)
      #x11(width = 11.75, height = 5.25)
      par(mfrow = c(1,3), mai = c(1.75,0.75,0.5,0.15))
      plot.new()
      boxplot(mise_STDEPDE, mise_EQUAL_SPLIT, mise_HEAT, mise_STLNPP,
    ...
    ## CPU TIMES ---------------------------------------------------------------
    pdf(paste0("pictures/", N, "_data/sim1_CPUtimes.pdf"), family = "serif", width = 11.75, height = 5.25)
    #x11(width = 11.75, height = 5.25)
    par(mfrow = c(1,3), mai = c(1.75,0.75,0.5,0.15))
    plot.new()
    ...
    plot(mesh, linewidth = 1) + geom_point(data = data.frame(x = mesh$nodes[,1], y = mesh$nodes[,2]),
                                           aes(x = x, y = y), color = "red3", size = 2)
    ggsave(filename = paste0("pictures/", N, "_data/sim1_mesh.pdf"),
    ...
    plot(mesh.eval, linewidth = 1) + geom_point(data = data.frame(x = mesh.eval$nodes[,1], y = mesh.eval$nodes[,2]),
                                                aes(x = x, y = y), color = "red3", size = 2)
    ggsave(filename = paste0("pictures/", N, "_data/sim1_mesh_eval.pdf"),
           plot = last_plot(), width = 8, height = 8)
    
    ## TRAJECTORIES FOLLOWED BY THE SOURCES OF INTENSITY -----------------------
    plot.trajectories(mesh, compute.sources1(spatstat.linnet))
    ggsave(filename = paste0("pictures/", N, "_data/sim1_trajectories.pdf"),
           plot = last_plot(), width = 8, height = 8)
...
# 
# lambda_space <- 10^seq(from = -2.25, to = -3.75, length.out = 7)
# lambda_time <- 10^seq(from = -3.25, to = -4.75, length.out = 7)
# 
